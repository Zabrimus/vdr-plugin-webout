...
WebEncoder2::WebEncoder2(const char* ffmpeg, const char* ffprobe, const char* in, const char* out, bool write2File) {
...
bool WebEncoder2::set_input_file(const char* input) {
...
// Logging functions
void WebEncoder2::logging(const char *fmt, ...)
{
    va_list args;
    fprintf( stderr, "LOG: " );
    va_start( args, fmt );
...
void WebEncoder2::log_packet(const AVFormatContext *fmt_ctx, const AVPacket *pkt)
{
...
void WebEncoder2::print_timing(char *name, AVFormatContext *avf, AVCodecContext *avc, AVStream *avs) {
    logging("=================================================");
    logging("%s", name);
...
int WebEncoder2::fill_stream_info(AVStream *avs, AVCodec **avc, AVCodecContext **avcc) {
...
int WebEncoder2::open_media(const char *in_filename, AVFormatContext **avfc) {
...
int WebEncoder2::init_audio_filter_graph(StreamingContext *decoder) {
...
int WebEncoder2::create_video_buffersrc_filter(StreamingContext *decoder, AVFilterContext **filt_ctx, AVFilterGraph *graph_ctx) {
    char strbuf[512];
    int err;
...
int WebEncoder2::create_video_realtime_filter(StreamingContext *decoder, AVFilterContext **filt_ctx, AVFilterGraph *graph_ctx) {
    int err;
    char strbuf[512];
...
int WebEncoder2::create_video_buffersink_filter(StreamingContext *decoder, AVFilterContext **filt_ctx, AVFilterGraph *graph_ctx) {
    int err;
...
int WebEncoder2::init_full_video_filter_graph(StreamingContext *decoder) {
    int err;
...
int WebEncoder2::prepare_decoder() {
...
int WebEncoder2::prepare_video_encoder(AVCodecContext *decoder_ctx, AVRational input_framerate) {
    int response;
...
int WebEncoder2::prepare_audio_encoder() {
...
int WebEncoder2::encode_video(AVFrame *input_frame) {
...
int WebEncoder2::encode_audio(AVFrame *input_frame) {
    AVPacket *output_packet = av_packet_alloc();
    if (!output_packet) {
        logging("could not allocate memory for output packet");
        return -1;
...
int WebEncoder2::transcode_audio(AVPacket *input_packet, AVFrame *input_frame) {
...
int WebEncoder2::transcode_video(AVPacket *input_packet, AVFrame *input_frame) {
...
std::thread WebEncoder2::transcode(int (*write_packet)(void *opaque, uint8_t *buf, int buf_size)) {
    return std::thread(&WebEncoder2::transcode_worker, this, write_packet);
}

int WebEncoder2::transcode_worker(int (*write_packet)(void *opaque, uint8_t *buf, int buf_size)) {
...
void WebEncoder2::pause_video() {
    pause_video_flag = true;

    kill(ffmpeg_pid, SIGSTOP);
}

void WebEncoder2::resume_video() {
    pause_video_flag = false;

    kill(ffmpeg_pid, SIGCONT);
}

void WebEncoder2::stop_video() {
...
void WebEncoder2::seek_video(const char* ms) {
    // TODO: implement me
    printf("Seek requested: %s\n", ms);
}

void WebEncoder2::speed_video(const char* speed) {
    // TODO: implement me
    printf("Speed requested: %s\n", speed);
}


int WebEncoder2::get_video_width() {
...
int WebEncoder2::get_video_height() {
    if (decoder->video_avcc) {
